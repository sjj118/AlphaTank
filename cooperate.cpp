...
namespace TankGame {
    ...
    public:
        ...
        bool IsCounter(int side, int tank) {
            if (!CanTankShootEachOther(side, tank))return false;
            int y = (tankY[side][tank] + tankY[!side][!tank]);
            int x = tankX[side][tank];
            int y1, y2;
            if (y & 1)y1 = y / 2, y2 = y / 2 + 1;
            else y1 = y / 2 - 1, y2 = y / 2 + 1;
        ...
        bool AtHome(int side, int tank) {
            if (side == Blue)return tankY[side][tank] < 4;
            else return tankY[side][tank] > 4;
        }

        bool AtEnemy(int side, int tank) {
            if (side == Red)return tankY[side][tank] < 4;
            else return tankY[side][tank] > 4;
        }
        ...
        int EstimateAttack(int side, int tank) {
            ...
            if (IsLink(tankX[side][tank], tankY[side][tank], tankX[!side][!tank], tankY[!side][!tank])) {
                int ret = 2 * (attackDis[!side][!tank] - attackDis[side][tank]);
                if (VerticalDis(side, tank) <= 0 ||
                    ...
                    if (CanTankShootEachOther(side, tank)) {
                        ...
                        if (c1 == -1 && c2 == -1);
                        else if (c1 == -1)ret -= c2 + 1;
                        else if (c2 == -1)ret += c1 + 1;
                        else ret += sgn(c1 - c2);
                    }
                    return 10000 * ret - 50;
                }
//                if (IsCounter(side, tank)) {
//                    return 0;
//                }
            }
        ...
        Action GetPattern(int side, int tank, int side2, int tank2) {
            for (int cycle = 1; cycle <= 4; ++cycle) {
                if (currentTurn > 2 * cycle) {
                    bool flag = true;
                    for (int i = 1; i <= 2 * cycle; ++i)if (hasDestroyBlock[side][tank][currentTurn - i])flag = false;
                    for (int i = 1; i <= 2 * cycle; ++i)if (hasDestroyBlock[side2][tank2][currentTurn - i])flag = false;
                    for (int i = 1; i <= 2 * cycle - cycle; ++i) {
    ...
    struct DecisionTree {
        ...
        static constexpr Action acts[2][9] = {
                {DownShoot, Down, LeftShoot, RightShoot, Left, Right, Stay, Up,   UpShoot},
                {UpShoot,   Up,   LeftShoot, RightShoot, Left, Right, Stay, Down, DownShoot}
        };
        ...
        int EstimateCross(Action act0) {
            Action pattern = field->GetPattern(!side, tank, side, tank);
            for (auto act1:acts[!side]) {
                if (pattern != Invalid && pattern != act1)continue;
                if (field->ActionIsValid(!side, tank, act1)) {
                    if (field->WillKill(!side, tank, act1, side, tank, act0))return (int) -1e8;
                    if (field->WillKill(side, tank, act0, !side, tank, act1))return (int) 1;
                }
            }
            return 0;
        }
            ...
            if (depth <= 3) {// quick judge
                for (auto act0:acts[side]) {
                    if (field->ActionIsValid(side, tank, act0)) {
                        ...
                        else if (!field->tankAlive[!side][!tank])tmp = (int) 1e8;
                        field->Revert();
                        if (tmp > beta) {
                            beta = tmp;
                            act = act0;
                ...
                if (field->ActionIsValid(side, tank, act0)) {
                    ...
                    for (auto act1:acts[!side]) {
                        ...
                        if (field->ActionIsValid(!side, !tank, act1)) {
                            ...
                            field->Revert();
                            if (depth == 0)tmp += EstimateCross(act0);
...
//                        if (!flag)break;
        ...
        Action GetAction() {
            ...
            if (value <= -10000 && (act == Invalid || field->Defensible(side, tank))) {
    ...
    pair<Action, Action> SubmitAction() {
